    ...
    for ($i=$pagemin; $i <= min($pagemax,$nbmessage); $i++){
      //Envoie des 50 derniers messages de la bases de données, 10 par page.
        ...
        if(($recuperedchat2[$nbmessage - $i]['user_id']) == ($_SESSION['user_id']) && !($recuperedchat2[$nbmessage - $i]['supp']) && !($recuperedchat2[$nbmessage - $i]['ban'])){
          ...
          echo('<br>');
          echo('<a href="modifiermessage.php?id='. $id .'" class="modifiermessage">Modifier</a>'); //envoie du message_id par l'URL pour modification
          echo('&nbsp <a href="supprimermessage.php?id='. $id .'" class="supprimermessage">Supprimer</a></p>'); //envoie du message_id par l'URL pour suppression
        } else {
          echo('</p>');
        }
...
<?php 
    ...
    for ($i=1; $i<=$nbpage; $i++){
      echo'<a href="event.php?p='. $i .'">Page ' . $i .'</a>&nbsp';
    }
    echo ' </p>' ;
    ?>
